...
function propertyCard(property) { const image = property.images?.[0] || property.image; const video = property.videos?.[0]; const type = property.type ? property.type : 'house'; const location = property.location || 'Nigeria'; const mediaNote = property.videos?.length ? `<span class="media-note">▶ ${property.videos.length} video${property.videos.length > 1 ? 's' : ''}</span>` : ''; const media = video ? `<video class="property-video" controls preload="metadata" playsinline poster="${image}"><source src="${video}"><a href="${video}" target="_blank" rel="noreferrer">Open property video</a></video>` : `<img class="property-image" src="${image}" alt="${property.title}" loading="lazy">`; return `<article class="property-card"><div class="property-media">${media}<span class="type-badge">${type}</span>${mediaNote}</div><div class="property-info"><p class="location">${location}</p><h3>${property.title}</h3><div class="price">${money(property.price)}</div><p class="description">${property.description}</p></div></article>`; }
let allProperties = [];
