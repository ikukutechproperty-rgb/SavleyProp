...
function renderProperties(properties) {
  ...
  gridTarget.querySelectorAll('.property-card').forEach((card) => window.clearTimeout(card._slideshowTimer));
...
function updateAccountUI() { const signedIn = Boolean(state.user); document.querySelectorAll('[data-open-auth]').forEach((button) => button.classList.toggle('hidden', signedIn)); document.querySelectorAll('.nav-user').forEach((target) => { target.textContent = signedIn ? `Hi, ${state.user.name}` : ''; target.classList.toggle('hidden', !signedIn); }); document.querySelectorAll('.nav-logout').forEach((button) => button.classList.toggle('hidden', !signedIn)); }
function signOut() { state.token = null; state.user = null; localStorage.removeItem('savleyToken'); localStorage.removeItem('savleyUser'); updateAccountUI(); closeModal('#guest-modal'); }
function setAuthMode(signUp) { state.signUp = signUp; const title = $('#auth-title'); const nameField = $('#name-field'); const switchButton = $('#auth-switch'); const message = $('#auth-message'); if (title) title.textContent = signUp ? 'Create account' : 'Sign in'; if (nameField) { nameField.classList.toggle('hidden', !signUp); const input = nameField.querySelector('input'); if (input) input.required = signUp; } if (switchButton) switchButton.textContent = signUp ? 'I already have an account' : 'Create an account'; if (message) { message.textContent = ''; message.classList.remove('success'); } }
async function handleAuth(event) { event.preventDefault(); const formElement = event.target; const button = formElement.querySelector('button[type="submit"]'); const message = $('#auth-message'); const wasSignUp = state.signUp; const form = new FormData(formElement); const payload = Object.fromEntries(form.entries()); if (button) button.disabled = true; if (message) { message.textContent = wasSignUp ? 'Creating your account...' : ''; message.classList.remove('success'); } try { const data = await api(`/api/auth/${wasSignUp ? 'signup' : 'login'}`, { method: 'POST', body: JSON.stringify(payload) }); state.token = data.token; state.user = data.user; localStorage.setItem('savleyToken', state.token); localStorage.setItem('savleyUser', JSON.stringify(state.user)); updateAccountUI(); closeModal('#auth-modal'); closeModal('#guest-modal'); formElement.reset(); if (state.user.role === 'admin') window.location.assign('/admin'); } catch (error) { if (message) message.textContent = error.message; } finally { if (button) button.disabled = false; } }
...
document.addEventListener('click', (event) => {
  ...
  const card = galleryButton.closest('.property-card');
  ...
  if (!item || !card) return;
  const selectedIndex = Number(galleryButton.dataset.galleryIndex);
  showPropertyMedia(card, property, selectedIndex);
  schedulePropertySlideshow(card, property, selectedIndex);
});
if ($('#auth-switch')) $('#auth-switch').addEventListener('click', () => setAuthMode(!state.signUp));
document.querySelectorAll('[data-open-auth]').forEach((button) => button.addEventListener('click', () => { setAuthMode(false); closeModal('#guest-modal'); const mobileNav = document.querySelector('.mobile-nav'); if (mobileNav) mobileNav.classList.remove('is-open'); const menuButton = document.querySelector('.menu-button'); if (menuButton) menuButton.setAttribute('aria-expanded', 'false'); openModal('#auth-modal'); }));
document.querySelectorAll('[data-guest-signin]').forEach((button) => button.addEventListener('click', () => { closeModal('#guest-modal'); setAuthMode(false); openModal('#auth-modal'); }));
document.querySelectorAll('[data-close-guest], [data-continue-guest]').forEach((button) => button.addEventListener('click', () => closeModal('#guest-modal')));
document.querySelectorAll('.nav-logout').forEach((button) => button.addEventListener('click', signOut));
...
if (document.body.dataset.page === 'admin') {
  ...
  updateAccountUI();
  window.setTimeout(() => { if (!state.user) openModal('#guest-modal'); }, 3200);
